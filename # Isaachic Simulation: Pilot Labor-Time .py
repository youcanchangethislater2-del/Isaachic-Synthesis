...
class ResourceBank:
    def __init__(self):
        self.waste_bin = {"Steel": 0, "Plastic": 0, "Wood": 0, "Plastic": 0, "Bread": 0, "Medicine": 0, "Water": 0, "Electricity": 0, "Healthcare": 0} 
    ...
    def tick(self, drought = False): 
        """The 'Earth' heals, but climate events can hinder it."""
        print("\n>>> METABOLIC TICK <<<")
        if drought: print("!!! CLIMATE EVENT: DROUGHT (Regrowth halved) !!!")
    ...
    def deplete(self, resource_name, amount):
        if resource_name in self.registry:
            ...
            # MISSING LOGIC: Move the used resource into the waste_bin
            if resource_name in self.waste_bin:
                self.waste_bin[resource_name] += usage
...
class CentralPlan:
    ...
    def calculate_suv(self):
        ...
        # Reset vouchers for the next year's cycle
        for name in self.products:
            self.products[name]["spent_vouchers"] = 0
...
# Define base demand for resources
base_demand = 100
...
for year in range(1, 11):
    ...
    # Let's trigger a Drought in Year 6 to test the brain's logic
    earth.tick(drought=(year == 6))
    plan.innovate()
    earth.recycle()
